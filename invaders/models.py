...
class Ship(GImage):
    ...
    def __init__(self,source0):
        """
        Initializes the class Ship (and its attributes).

        Parameter source0: specifies an image file in the Images folder
        Precondition: source0 is a string of the file name
        """
        super().__init__(x=GAME_WIDTH/2,y=SHIP_BOTTOM+SHIP_HEIGHT/2,width=
                         SHIP_WIDTH,height=SHIP_HEIGHT,source=source0)
    ...
    def collides(self,bolt):
        ...
        """
        if self.contains((bolt.x-BOLT_WIDTH/2,bolt.y-BOLT_HEIGHT/2)) or \
...
class Alien(GImage):
    ...
    def __init__(self,x0,y0,source0):
        """
        Initializes the class Alien (and its attributes).

        Parameter x0: the horizontal coordinate of the object center.
        Precondition: x0 must be a number

        Parameter y0: the vertical coordinate of the object center.
        Precondition: y0 must be a number

        Parameter source0: specifies an image file in the Images folder
        Precondition: source0 is a string of the file name
        """
        super().__init__(x=x0,y=y0,width=ALIEN_WIDTH,height=ALIEN_HEIGHT,
                         source=source0)
...
class Bolt(GRectangle):
    ...
    OTHER ATTRIBUTES:
        _playerbolt: Whether or not the bolt is fired by the player [bool]
    """
    ...
    def __init__(self,x0,y0):
        """
        Initializes the class Bolt and its attributes.

        Parameter x0: the horizontal coordinate of the object center.
        Precondition: x0 must be a number
        """
        super().__init__(x=x0,y=y0,width=BOLT_WIDTH,height=BOLT_HEIGHT,
                         fillcolor = 'black')
    ...
    def isPlayerBolt(self):
        """
        Returns: True if bolt is fired by player, False otherwise
        """
        return self._playerbolt
